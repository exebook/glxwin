wstrings a2s(const Arguments& args) { // TODO: get rid of this one day
	wstrings R;
	for (int i = 0; i < args.Length(); i++) {
		HandleScope handle_scope;
		String::Value stri(args[i]);
...
function(ijs_fopen) {
	wstrings L = a2s(a);
	str P1 = L[0];
	long R = (long)fopen(*P1, "rb");
	// TODO: use map here, because pn 64 bit pointer can be dangerous
	double X = R;
	if (X != R) printf("YAUI design error: fopen() result '%p' does not fit into double %d!", R, X);
	return Number::New(X);
}
function(ijs_fclose) {
	wstrings L = a2s(a);
...
function(ijs_fread_utf16) {
	str R = read_data(a, 2);
	return String::New((uint16_t*)*R, !R / 2);
}
function(v8_exit) {
