...
Handle<Object> v8_error_as_object(TryCatch* try_catch) {
...
wstr v8_error_as_string(TryCatch* t) {
	wstr R;
	HandleScope handle_scope;
	R / cvt(t->Exception());
	Handle<Message> m = t->Message();
	if (!m.IsEmpty()) {
...
// v8_exe and v8_exec - do the same, but first returns error or result as wstr, and second as Object
bool v8_exe_error;
...
void js_call(char * name, Handle<Function> f, int argc, Handle<Value> *args) {
	TryCatch try_catch;
	Handle<Value> result = f->Call(context->Global(), argc, args);
	if (result.IsEmpty()) {
		str s = v8_error_as_string(&try_catch);
		printf("%s error: %s\n", name, *s);
	}
}
